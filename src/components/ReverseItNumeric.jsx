...
import { reverseItNumericFetch } from "../services/services";

const ReverseItNumeric = () =>
{
    ...
    const SolutionButton = async () =>
    {
        ...
        {
            setCondition( false );
            setBoxText( await reverseItNumericFetch( numericInput ) );
        }
    }

    return (
        <div className="bg-[url('/assets/Uranus.jpg')] bg-no-repeat bg-cover bg-center m-0">
            <div className="h-[100vh] grid grid-rows-9">
                <div className="grid">
                    ...
                    <div className="text-white flex justify-end text-[64px] font-bold mt-[45px] mr-[85px] text-right row-start-1 max-[720px]:text-[32px]">
                        <h1>Reverse It<br/>Numeric</h1>
                    </div>
                </div>

                <div className="row-start-5 flex justify-center max-[720px]:row-start-4">
                    <div className={`text-white bg-black/60 w-[277px] h-[175px] border rounded-[5px] ml-[10px] shadow-[0px_0px_0.2em_0px_rgba(0,0,0)] shadow-white m-0 overflow-y-auto ${condition ? "hidden" : ""}`}>
...
export default ReverseItNumeric;